 ...
 * @mainpage 框架介绍
 *
 * @note APF(Application Plugin Framework) 是一套轻量级的插件框架, 采用C++编写，支持windows、linux等平台。<BR/>
 * 本框架的主要思想是使用接口模块化编程，使用依赖注入方式动态创建对象。<BR/>
 * 应用程序可以基于APF插件框架进行快速开发，APF插件框架中的插件既可以单独使用，又可以灵活组合使用。
 * @par APF插件框架的特点有：
 * @par a) 接口定义简单灵活
 *    采用普通的C++接口，即由纯虚函数组成的结构体，不需要特殊的基类，不需要宏和UUID申明；同时可以使用C++的各种变量类型，不受COM接口那样的约束。例如下面的接口定义：<BR/>
 * @code
class IExample {
